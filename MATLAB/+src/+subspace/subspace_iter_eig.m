function [V, D] = subspace_iter_eig(A, n, k, varargin)
%% function [V, D] = subspace_iter_eig(A, n, k, varargin)
...
%               Optional. Should accept a matrix and orthogonal basis.
%               In the form [V, D] = function_rr(A, Q), where Q is orthogonal.
%               Default creates a wrapper around src.rr.eig_rr with the specified precision.
%               Custom functions should handle precision internally.
...
%   [V, D] = src.subspace.subspace_iter_eig(A, size(A, 1), 10, 'maxiter', 10, 'step_size', 2);
   ...
   for i = 1:maxiter
      for j = 1:step_size
         V = matvec(A, 'N', V, 'precision_compute', precision_compute, 'precision_output', precision_output);
         V = src.mvops.scalecols(V, 'precision_compute', precision_compute, 'precision_output', precision_output);
      end
      [Q, ~] = function_orth(V);
      [V, D] = function_rr(A, Q);
...
function [precision_compute, precision_output, seed, maxiter, step_size, function_orth, function_rr, matvec, check_params] = parse_options(varargin)
   ...
   if ~exist('function_rr', 'var')
      function_rr = @(A, Q) src.rr.eig_rr(A, Q, 'precision_compute', precision_compute, 'precision_output', precision_output);
   end
   if ~exist('matvec', 'var')
