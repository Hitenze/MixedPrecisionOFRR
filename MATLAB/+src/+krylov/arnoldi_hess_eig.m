function [V, D] =  arnoldi_hess_eig(A, n, k, varargin)
%% function [V, D] = arnoldi_hess_eig(A, n, k, varargin)
...
%   [V, D] = src.subspace.subspace_iter_eig(A, size(A, 1), 10, 'maxiter', 10, 'step_size', 2);
   ...
   [precision_compute, precision_output, precision_eigenvalues, precision_A_matvec_compute, precision_A_matvec_output, seed,maxiter, kdim, select_tol, generalized_tol, matvec, check_params] = parse_options(varargin{:});
   ...
   while tits < maxiter
      ...
      [~, l] = max(abs(V(:,1)));
      perm(1) = l;
      if abs(V(l,1)) < select_tol
         break;
      else
         V(:,1) = precision_output(precision_compute(V(:,1)) / precision_compute(V(l,1)));
      end
      for i = 1:kdim
         i1 = i + 1;
         ...
         for j = 1:i
            V(:, i1) = precision_output(precision_compute(V(:, i1)) - precision_compute(V(perm(j),i1)) * precision_compute(V(:,j)));
         end

         [~, l] = max(abs(V(:,i1)));
         perm(i1) = l;
         if abs(V(l,i1)) < select_tol
            tits = maxiter;
            break;
         else
            V(:,i1) = precision_output(precision_compute(V(:,i1)) / precision_compute(V(l,i1)));
         end
         tits = tits + 1;
      end
...
function [precision_compute, precision_output, precision_eigenvalues, precision_A_matvec_compute, precision_A_matvec_output, seed, maxiter, kdim, select_tol, generalized_tol, matvec, check_params] = parse_options(varargin)
   if nargin > 1
      for i = 1:2:nargin
         switch varargin{i}
