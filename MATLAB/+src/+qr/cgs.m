function varargout = mgs(A, varargin)
   ...
   %% function [Q,R,select] = cgs(A, varargin)
   % Compute the QR factorization of A using classical Gram-Schmidt orthogonalization.
   %
   % Inputs:
   ...
   %   [Q,R,select] = src.qr.mgs(A, 'reorth_tol', -1); % No reorthogonalization
      ...
      r_vec = precision_output(zeros(n,1));
      ...
      for i = 1:n
         ...
         % CGS, note that we don't use any vectorized operations here
         for j = 1:i-1
            if select(j)
         ...
         while(reorth_tol >= 0 && (t >= orth_tol && t < reorth_tol*normv))
            ...
            for j = 1:i-1
               if select(j)
                  r_vec(j) = src.mvops.dot(v, Q(:,j), 'precision_compute', precision_compute, 'precision_output', precision_output);
               end
            end
            for j = 1:i-1
               if select(j)
                  v = precision_output(precision_compute(v) - precision_compute(r_vec(j))*precision_compute(Q(:,j)));
                  R(j,i) = precision_output(precision_compute(R(j,i)) + precision_compute(r_vec(j)));
               end
            end
            t = src.mvops.norm2(v, 'precision_compute', precision_compute, 'precision_output', precision_output);
         end
